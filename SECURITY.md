...
const DATA_VERSION = "20260509-14";
...
function makeLaunchAudit() {
  ...
  const blockers = makeLaunchBlockers({ companyRows, starterRows, packet, realCoverage, realSlots, totalSlots, radar, portfolio, calendar, briefing, taskBoard, sprintPlan, icMemo, claimTrace, answerQuality });
  ...
  const score = Math.round(
    ...
    + taskBoardScore * 0.05
    + sprintScore * 0.04
    + (packet.hasBrief ? packet.score : 0) * 0.05
    + icMemoScore * 0.08
    + claimTraceScore * 0.06
    + answerQualityScore * 0.06
    + Math.max(0, 100 - blockers.length * 12) * 0.04
  );
  ...
  return {
    releaseLabel: "v40 Answer Quality Lab",
    generatedAt: new Date().toISOString(),
    statusLabel,
    statusClass,
    score,
...
function makeLaunchBlockers({ companyRows, starterRows, packet, realCoverage, realSlots, radar, portfolio, calendar, briefing, taskBoard, sprintPlan, icMemo, claimTrace, answerQuality }) {
  const blockers = [];
  if (!packet.hasBrief) {
    blockers.push({
      severity: "High",
...
function makeLaunchTestPlan() {
  return [
    "After uploading the ZIP contents to GitHub, confirm the top status pill says Answer quality v40.",
...
function exportLaunchAuditPack() {
  const audit = makeLaunchAudit();
  const date = new Date().toISOString().slice(0, 10);
  const filename = `niveshscope-launch-audit-v40-${date}.json`;
...
function makeLaunchUploadChecklistMarkdown(audit) {
  ...
  return [
    "# NiveshScope v40 Upload Checklist",
    "",
    ...
    "## Before Upload",
    "",
    "- Upload the contents of the v40 ZIP to the GitHub repository root.",
